          ...
          >
            {slides.map((slide, i) => (
              <div key={i} className="min-w-full h-full relative overflow-hidden">
                ...
                {/* Content */}
                <div className={`absolute inset-0 z-10 flex flex-col justify-end p-8 md:p-24 ${direction === 'rtl' ? 'items-start text-right' : 'items-start text-left'}`}>
                  <div className={`transition-all duration-1000 transform max-w-4xl ${activeIndex === i ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
                    ...
                    <h1 className="text-4xl md:text-7xl font-black text-white mb-6 tracking-tighter leading-[0.9] drop-shadow-2xl">
                      {slide.title}
                    </h1>
                    <p className="text-sm md:text-2xl text-gold-light font-medium leading-relaxed drop-shadow-lg max-w-2xl opacity-90">
                      {slide.subtitle}
                    </p>
