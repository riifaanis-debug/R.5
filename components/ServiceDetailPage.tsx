    ...
    try {
      const response = await fetch("/api/submit-request", {
        ...
        body: safeStringify({
          ...
          mobile: phone,
          summary: message,
          bank: serviceType, // Using serviceType as bank for the general form
      ...
      if (response.ok) {
        ...
        setError(data.message || "عذراً، حدث خطأ أثناء إرسال الطلب. يرجى المحاولة مرة أخرى.");
      }
    } catch (err) {
  ...
  if (!content) return (
    <PageLayout title="خطأ">
      <div className="text-center py-10 w-full">
        ...
        <a href="#/" className="text-gold underline mt-4 block">العودة للرئيسية</a>
      </div>
    </PageLayout>
  );
  ...
  return (
    <PageLayout title={content.title}>
      <div className="space-y-8 w-full max-w-full">
        ...
        {selectedSubService && (
          <div className="animate-in fade-in slide-in-from-bottom-6 duration-500 space-y-6 w-full max-w-full overflow-hidden">
            ...
            <div className="flex flex-col gap-4 mt-8">
               ...
               <Button onClick={handleGoHome} variant="ghost" className="w-full h-12 gap-3 border-gold/30 hover:border-gold">
                  <ArrowRight size={18} />
