...
const WaiveRequestForm: React.FC<WaiveRequestFormProps> = ({ onClose, prefill }) => {
  ...
  const [documents, setDocuments] = useState(prefill?.documents?.length > 0 ? prefill.documents.map((d: any) => ({ ...d, id: d.id || Date.now() + Math.random() })) : [{ id: 1, type: '', file: null }]);
  ...
  useEffect(() => {
    const handleWindowClick = (e: MouseEvent) => {
      console.log("Window clicked at:", e.clientX, e.clientY, "Target:", e.target);
    };
    ...
    try {
      ...
      const processedDocs = await Promise.all(
        documents.map(async (doc, idx) => {
          ...
          // Use document.getElementById or similar if needed, but form query selector should work if e is passed
          // Since we might not have 'e.target', we use document query
          const fileInput = document.querySelector(`input[name="docFile_${idx}"]`) as HTMLInputElement;
          const file = fileInput?.files?.[0];
          if (file) {
            const base64 = await fileToBase64(file);
            const typeSelect = document.querySelector(`select[name="docType_${idx}"]`) as HTMLSelectElement;
            return {
              type: typeSelect?.value || doc.type,
              fileName: file.name,
              mimeType: file.type,
              content: base64
            };
      ...
      const data = {
        ...
        attachments: processedDocs.filter(d => d !== null)
      };

      // Submit via backend API to handle both DB insert and Email notification
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.message || "فشل إرسال الطلب");
      }
      ...
      alert(error.message || "حدث خطأ أثناء الإرسال، يرجى المحاولة لاحقاً.");
    } finally {
  ...
  const addDocument = () => setDocuments([...documents, { id: Date.now(), type: '', file: null }]);
        ...
        <form onSubmit={handleSubmit} noValidate className="bg-white rounded-[16px] border border-gold/45 shadow-[0_18px_45px_rgba(0,0,0,0.06)] p-5 sm:p-8">
          ...
          <div className="mt-8 mb-6 bg-gradient-to-b from-white to-[#FCFAF4] rounded-[16px] border border-gold/30 p-4 shadow-inner">
             ...
             <div className="mt-4 pt-3 border-t border-gold/10 flex justify-between items-center">
                ...
                <span className="text-[14px] font-extrabold text-brand tabular-nums">{totalAmount.toLocaleString()} ر.س</span>
             </div>
          </div>
             ...
             <div className="space-y-2">
                {documents.map((doc, idx) => (
                   <div key={doc.id} className="flex gap-2 items-center">
                      <select name={`docType_${idx}`} className="w-1/3 p-2 rounded-[10px] border border-gold/20 text-[11px] bg-white">
                      ...
                      <input type="file" name={`docFile_${idx}`} className="flex-1 text-[11px] text-muted" />
                      {documents.length > 1 && (
