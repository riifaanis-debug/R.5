...
const ContractPage: React.FC<ContractPageProps> = ({ submissionId, onClose }) => {
  ...
  const totalDebt = submission?.data?.products?.reduce((acc: number, p: any) => acc + (Number(p.amount) || 0), 0) || 0;
    ...
    if (signatureData) {
      try {
        const res = await fetch('/api/submit-signature', {
          ...
          body: safeStringify({ submission_id: submissionId, signature_data: signatureData })
        });
        ...
        <div className="max-w-[210mm] min-h-[297mm] mx-auto bg-white shadow-[0_10px_30px_rgba(0,0,0,0.05)] border border-gray-200 p-6 sm:p-10 relative overflow-hidden font-['Tajawal'] print:shadow-none print:border-none print:p-0 print:w-full">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6 relative border-b-2 border-[#22042C] pb-3">
            <div className="text-right">
              <img src="https://mrkzgulfup.com/uploads/17723308921271.png" alt="Rifans Logo" className="h-20 mb-2" referrerPolicy="no-referrer" />
          ...
          <div className="space-y-4 text-right dir-rtl relative text-[10.5px] leading-[1.6] text-[#22042C]">
            ...
            <section>
              ...
              <p className="mb-1.5">
                {isRescheduling 
                  ...
                  : `يفوض الطرف الثاني بموجب هذا العقد تفويضاً صريحاً ومباشراً وقابلة للتنفيذ للطرف الأول في استلام وتقديم ومتابعة طلب الإعفاء المقدم من الطرف الثاني لدى ${submission.data.bank || 'الجهة المالية'}، وذلك فيما يتعلق بمنتجات التمويل الموضحة أدناه:`
                }
              </p>
              ...
              <div className="bg-white border border-gray-100 p-3 rounded-lg space-y-2 text-[11px]">
                ...
                {submission.data.products?.map((product: any, idx: number) => (
                ...
                <p className="font-black text-brand pt-1 text-[12px] border-t-2 border-brand/10">إجمالي المديونية: {totalDebt.toLocaleString()} ريال سعودي</p>
              </div>
            </section>
            ...
            <section>
              ...
              <p>
                {isRescheduling
                  ? 'يشمل التفويض الممنوح للطرف الأول الصلاحيات التالية: الاطلاع على المستندات والبيانات المالية، التواصل مع البنوك والجهات التمويلية، رفع الطلبات ومتابعتها، وإعداد المذكرات النظامية والحضور النظامي عند الحاجة.'
            ...
            <section>
              ...
              <p>
                {isRescheduling
                  ? 'لا تستحق أتعاب الطرف الأول إلا بعد صدور قرار الموافقة على إعادة جدولة المنتجات التمويلية وإتمام الإجراءات ذات العلاقة. وفي حال صدور القرار يستحق الطرف الأول أتعاباً مقطوعة قدرها: 2,000 ريال سعودي فقط. وفي حال عدم قبول الطلب، لا يحق للطرف الأول المطالبة بأي أتعاب.'
      ...
      {isSuccess && (
        <div className="fixed inset-0 bg-white/95 backdrop-blur-md flex items-center justify-center z-[100] animate-in fade-in duration-500 print-hidden">
          <div className="text-center p-6 sm:p-10 max-w-sm">
            ...
            <h3 className="text-[20px] sm:text-[24px] font-black text-[#22042C] mb-3 font-['Tajawal']">تم التوقيع بنجاح</h3>
            <p className="text-[13px] sm:text-[14px] text-muted leading-relaxed font-['Tajawal']">
              شكراً لك {user?.name.split(' ')[0]}، تم اعتماد توقيعك الإلكتروني بنجاح وإرسال النسخة الموقعة إلى النظام.
            </p>
