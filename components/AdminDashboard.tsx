...
type DashboardTab = 'home' | 'stats' | 'clients' | 'waive_requests' | 'rescheduling_requests' | 'service_requests' | 'contracts' | 'notifications';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onClose }) => {
  ...
  const fetchSubmissionHistory = async (id: string) => {
    ...
    try {
      // History could be implemented in server.ts if needed
  ...
  const handleConfirmSendContract = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('/api/admin/send-contract', {
        method: 'POST',
        headers: { 
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        },
        body: safeStringify({ userId, submissionId })
      });
      ...
      if (response.ok) {
        ...
        fetchAllData(); // Refresh to show new status and contract
      }
    } catch (err) {
      console.error(err);
    }
          ...
          <tbody className="divide-y divide-gold/5">
            {filteredUsers.map(u => (
              <tr key={u.id} className="hover:bg-gold/5 transition-colors group">
                ...
                <td className="p-4 text-xs font-mono text-muted">{u.fileNumber || '---'}</td>
                <td className="p-4 text-xs text-brand dark:text-gray-300">{u.national_id || u.nationalId}</td>
                <td className="p-4 text-xs text-brand dark:text-gray-300" dir="ltr">{u.phone || u.mobile}</td>
                <td className="p-4 text-xs text-muted">{new Date(u.created_at || '').toLocaleDateString('ar-SA')}</td>
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    <button 
                    ...
                    <button 
                      onClick={() => {
                        ...
                        if (sub) {
                          ...
                          fetchSubmissionHistory(sub.id);
                        } else {
                          alert('لا يوجد طلبات نشطة لهذا العميل');
                        }
                      }}
                      className="p-2 hover:bg-gold/10 text-gold rounded-lg transition-colors" 
                      title="فتح الطلب"
          ...
          <table className="w-full text-right border-collapse">
            <thead>
              <tr className="text-xs font-bold text-muted bg-gray-50 dark:bg-black/20 border-b border-gold/10">
                <th className="p-4">العميل</th>
                <th className="p-4">رقم الملف</th>
                <th className="p-4">الجهة التمويلية</th>
                <th className="p-4">مبلغ المديونية</th>
            ...
            <tbody className="divide-y divide-gold/5">
              {requests.map(s => (
                <tr key={s.id} className="hover:bg-gold/5 transition-colors group">
                  ...
                  <td className="p-4 text-xs font-mono text-muted">{s.id}</td>
                  <td className="p-4 text-xs text-brand dark:text-gray-300">{s.data?.bank || '---'}</td>
                  <td className="p-4 text-xs font-bold text-brand dark:text-white">
                    {s.data?.totalAmount ? `${parseFloat(s.data.totalAmount).toLocaleString()} ر.س` : '---'}
                  </td>
                  <td className="p-4 text-xs text-muted">{new Date(s.timestamp).toLocaleDateString('ar-SA')}</td>
                  <td className="p-4">
                    <select 
                      value={s.status}
        ...
        <table className="w-full text-right border-collapse">
          <thead>
            <tr className="text-xs font-bold text-muted bg-gray-50 dark:bg-black/20 border-b border-gold/10">
              ...
              <th className="p-4">تاريخ التوقيع</th>
              <th className="p-4">الإجراءات</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gold/5">
            {filteredContracts.map(c => (
              <tr key={c.id} className="hover:bg-gold/5 transition-colors group">
                ...
                <td className="p-4 text-xs font-mono text-muted">{c.file_number}</td>
                <td className="p-4 text-xs text-brand dark:text-gray-300">
                  {c.type === 'waive_request' ? 'عقد إعفاء' : 'عقد جدولة'}
                </td>
                <td className="p-4 text-xs text-muted">{new Date(c.created_at).toLocaleDateString('ar-SA')}</td>
                <td className="p-4">
                  <span className={`text-[10px] font-bold px-3 py-1 rounded-full border ${
                    c.signed_at ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100'
                  }`}>
                    {c.signed_at ? 'تم التوقيع' : 'في انتظار التوقيع'}
                  </span>
                </td>
                <td className="p-4 text-xs text-muted">
                  {c.signed_at ? new Date(c.signed_at).toLocaleString('ar-SA') : '---'}
                </td>
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    <button 
                    ...
                    {!c.signed_at && (
                      <button 
                        onClick={() => sendContract(c.user_id, c.submission_id)}
                        className="p-2 hover:bg-gold/10 text-gold rounded-lg transition-colors" 
                        title="إعادة إرسال"
                      >
                        <RefreshCw size={16} />
  ...
  const renderNotifications = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="space-y-4">
        {notifications.map(n => (
          <div key={n.id} className="flex items-start gap-4 p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 hover:border-gold/30 transition-all group">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${
              n.type === 'status_update' ? 'bg-blue-100 text-blue-600' : 
              n.type === 'contract_signature' ? 'bg-rose-100 text-rose-600 border border-rose-200' : 'bg-gold/10 text-gold'
            }`}>
              {n.type === 'status_update' ? <RefreshCw size={18} /> : 
               n.type === 'contract_signature' ? <PenTool size={18} /> : <Bell size={18} />}
            </div>
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-brand dark:text-gold">{n.user_name}</span>
                  <span className="w-1 h-1 rounded-full bg-gray-300"></span>
                  <span className="text-[10px] text-muted">{n.title}</span>
                </div>
            ...
            <button className="p-2 opacity-0 group-hover:opacity-100 text-muted hover:text-rose-500 transition-all">
              <Trash2 size={16} />
            </button>
          </div>
        ))}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                  ...
                  <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <InfoItem icon={<TrendingUp size={14} />} label="إجمالي الالتزامات" value={`${parseFloat(selectedSubmission.data?.totalAmount || '0').toLocaleString()} ر.س`} isBold />
                        <InfoItem icon={<Calendar size={14} />} label="تاريخ التقديم" value={new Date(selectedSubmission.timestamp).toLocaleString('ar-SA')} />
                        <InfoItem icon={<Hash size={14} />} label="رقم الطلب" value={selectedSubmission.id} />
                      </div>
                    </div>

                    {/* Products Table */}
                    {selectedSubmission.data?.products && (
                      <div className="space-y-4">
                            ...
                            <tbody className="divide-y divide-gold/5">
                              {selectedSubmission.data.products.map((p: any, i: number) => (
                                <tr key={i}>
                                  ...
                                  <td className="p-3 font-bold text-brand dark:text-white">{parseFloat(p.amount).toLocaleString()} ر.س</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                    ...
                    {selectedSubmission.data?.attachments && selectedSubmission.data.attachments.length > 0 && (
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {selectedSubmission.data.attachments.map((att: any, i: number) => (
                            <div key={i} className="flex items-center justify-between bg-gray-50 dark:bg-white/5 p-4 rounded-2xl border border-gold/10 hover:border-gold/30 transition-all group">
                              <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-xl bg-gold/10 text-gold flex items-center justify-center">
                                  <Download size={18} />
                                </div>
                                <div className="min-w-0">
                                  <div className="text-xs font-bold text-brand dark:text-white truncate">{att.type}</div>
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="bg-gray-50 dark:bg-white/5 p-6 rounded-[24px] border border-gold/10 space-y-4">
                      ...
                      <button 
                        onClick={() => sendContract(selectedSubmission.user_id, selectedSubmission.id)}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="relative pr-4 space-y-6">
                        ...
                        {isHistoryLoading ? (
                          <div className="text-center py-4 text-xs text-muted">جاري تحميل السجل...</div>
                        ) : (
                          submissionHistory.map((h, i) => (
                            <div key={h.id} className="relative pr-6">
                              ...
                              <div className="text-xs font-bold text-brand dark:text-white mb-1">{h.action}</div>
                              <div className="flex items-center justify-between text-[10px] text-muted">
                                <span>بواسطة: {h.admin_name}</span>
            ...
            >
              ...
              <div className="p-2 sm:p-6 flex-1 overflow-y-auto custom-scrollbar print-container">
                {(() => {
                  ...
                  const totalDebt = sub.data?.products?.reduce((acc: number, p: any) => acc + (Number(p.amount) || 0), 0) || 0;
                    ...
                    <div className="bg-white dark:bg-white/5 rounded-xl border border-gold/10 p-4 sm:p-12 shadow-inner relative font-['Tajawal'] min-h-[800px]">
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-8 relative border-b-4 border-[#22042C] pb-4">
                        <div className="text-right">
                          <img src="https://mrkzgulfup.com/uploads/17723308921271.png" alt="Logo" className="h-20 mb-3" referrerPolicy="no-referrer" />
                      ...
                      <div className="space-y-4 text-right dir-rtl relative text-[10px] leading-[1.6] text-[#22042C] dark:text-gray-200">
                        ...
                        <div className="space-y-6">
                          <h2 className="text-center text-[14px] font-black text-[#22042C] border-b-2 border-gold pb-1 inline-block">بنود العقد التفصيلية</h2>
                          ...
                          <section>
                            ...
                            <p className="mb-1.5">
                              {isRescheduling 
                                ? 'يفوض الطرف الثاني بموجب هذا العقد تفويضاً صريحاً ومباشراً وقابلاً للتنفيذ للطرف الأول في استلام وتقديم ومتابعة طلب إعادة جدولة المنتجات التمويلية الخاصة به لدى الجهات التمويلية والبنوك.'
                            ...
                            <div className="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 p-3 rounded-lg space-y-2 text-[10px]">
                              ...
                              {sub.data?.products?.map((product: any, idx: number) => (
                                <div key={idx} className="space-y-1 border-t border-gray-50 dark:border-white/5 pt-1 first:border-0 first:pt-0">
                                  ...
                                  <p><strong>المبلغ:</strong> {Number(product.amount).toLocaleString()} ريال سعودي</p>
                                </div>
                              ))}
                              <p className="font-black text-brand dark:text-gold pt-1 text-[11px] border-t-2 border-brand/10">إجمالي المديونية: {totalDebt.toLocaleString()} ريال سعودي</p>
                            </div>
                          </section>
                        ...
                        <div className="mt-8 pt-4 border-t-2 border-[#22042C] grid grid-cols-2 gap-6 relative">
                          <div className="text-center space-y-2">
                            ...
                            <div className="h-20 flex items-center justify-center relative overflow-hidden">
                              <img 
                                src="https://j.top4top.io/p_3716if7fq1.jpg" 
                                alt="Stamp" 
                        ...
                        {/* Footer Notice */}
                        <div className="mt-6 text-center border-t border-gray-100 dark:border-white/10 pt-3">
              ...
              <div className="p-3 sm:p-5 border-t border-gold/10 bg-gray-50 dark:bg-black/20 flex items-center justify-end gap-2 print-hidden">
                <Button variant="outline" size="sm" onClick={() => setSelectedContract(null)} className="text-[10px] sm:text-xs h-7 sm:h-9">إغلاق</Button>
                <Button 
                  variant="outline" 
                  size="sm" 
        ...
        {isConfirmingSendContract && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-md bg-white dark:bg-[#12031a] rounded-[32px] shadow-2xl overflow-hidden p-8 text-center"
            >
...
const StatCard = ({ icon, label, value, color }: { icon: React.ReactNode, label: string, value: number, color: string }) => (
  <Card className="p-3 sm:p-4 border-gold/10 hover:border-gold/30 hover:shadow-lg transition-all group overflow-hidden relative">
    ...
    <div className="text-xl sm:text-2xl font-black text-brand dark:text-white tracking-tight">{value.toLocaleString()}</div>
  </Card>
);
...
const InfoItem = ({ icon, label, value, isLtr, isBold }: { icon: React.ReactNode, label: string, value: any, isLtr?: boolean, isBold?: boolean }) => (
  <div className="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10">
    <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2 text-muted">
      {icon}
      <span className="text-[8px] sm:text-[10px] font-bold uppercase tracking-wider">{label}</span>
    </div>
    <div className={`text-[10px] sm:text-xs ${isBold ? 'font-bold text-brand dark:text-gold sm:text-sm' : 'text-brand dark:text-white'} ${isLtr ? 'font-mono' : ''}`} dir={isLtr ? 'ltr' : 'rtl'}>
      {value || '---'}
    </div>
  </div>
);
